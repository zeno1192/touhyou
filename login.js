...
const formTitle = document.getElementById('formTitle'); // タイトルを動的に変更
...
toggleButton.addEventListener('click', () => {
  if (loginForm.classList.contains('hidden')) {
    ...
    formTitle.textContent = "ログイン"; // タイトル変更
    toggleButton.textContent = "新規登録はこちら"; // ボタンテキスト変更
  } else {
    // ログイン -> 新規登録に切り替え
    loginForm.classList.add('hidden');
    registerForm.classList.remove('hidden');
    formTitle.textContent = "新規登録"; // タイトル変更
    toggleButton.textContent = "ログインはこちら"; // ボタンテキスト変更
  }
});
